...
# slug -> AsciiDoc body (subsections use === / ====)
MODULE_EXTENDED: dict[str, str] = {
    "index": """
...
def _default_extended(slug: str) -> str:
...
def extended_section_body(slug: str) -> str:
    if slug in MODULE_EXTENDED:
        body = MODULE_EXTENDED[slug]
        if body:
            return body.strip()
    return _default_extended(slug).strip()
